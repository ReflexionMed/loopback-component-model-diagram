...
/**
 * Handle the referencesMany non-polymorphic relation.
 */
function handleReferencesMany(nodes, node, relation) {
  const destination = getOrCreateNodeEntry(nodes, new Node(relation.modelTo.modelName));
  const rel = new Relation(node, destination, 'referencesMany');
  node.addRelation(rel);
}
    ...
    if (node.relations.length > 0) {
      node.relations.forEach((rel) => {
        if (rel.type === 'belongsTo' || rel.type === 'generalize') {
          nomnomlSource += printConnection(rel, '-> belongsTo / generalize ');
        } else if (rel.type === 'hasOne') {
          if (rel.polymorphic) {
            nomnomlSource += printConnection(rel, '-- hasOne ');
          } else {
            nomnomlSource += printConnection(rel, '- hasOne ');
          }
        } else if (rel.type === 'hasMany') {
          if (rel.polymorphic) {
            nomnomlSource += printConnection(rel, ' hasMany --:>');
          } else {
            nomnomlSource += printConnection(rel, ' hasMany <-');
          }
        } else if (rel.type === 'hasAndBelongsToMany') {
          if (rel.polymorphic) {
            nomnomlSource += printConnection(rel, '<--> hasAndBelongsToMany ');
          } else {
            nomnomlSource += printConnection(rel, '<-> hasAndBelongsToMany ');
          }
        } else if (rel.type === 'referencesMany') {
          if (rel.polymorphic) {
            nomnomlSource += printConnection(rel, '<--:> referencesMany ');
          } else {
            nomnomlSource += printConnection(rel, '<--:> referencesMany ');
          }
        }
      ...
      Object.keys(model.relations).forEach((relationName) => {
        ...
        if (!containsObject(relationsToIgnore, rel)) {
          // it's not a relation to ignore
            ...
            // hasOne relation
          if (rel.type === 'referencesMany' && !rel.polymorphic) {
            handleReferencesMany(nodes, node, rel);
          }
